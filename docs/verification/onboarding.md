...
|---|---|---|---|
| First setup from a clean checkout (FastAPI) | A/H | pass | The agent read the skill, ran `lyra setup --json`, and read the docs and scripts. It then wrote 9 `dev` actions: Compose services with a `docker compose stop` cleanup (never `down -v`), prestart, backend, frontend, lint, tests, and prek. It validated and applied them and appended 3 ignore rules. It reported what it could not verify. 14 turns. |
...
| Save a one-off command as a tool (UC-02) | A/E | pass | Session A worked the answer out once with `lyra exec … python3 -c '<ast script>'`. It then saved `api-routes.list` with an `api-routes.endpoints` table and 2 optional inputs, validated, and applied (revision 3). |
...
| Add a typed panel live, without changing Core | E/H | pass | Asked for a table of Compose services and ports, the agent wrote `compose-ports` (manifest plus Python). It validated, applied, and ran it; the rows match the Compose files. Bad input gave SCHEMA_INVALID. The `lyra` binary was unchanged. VALUE-04. |
...
| Normal end keeps data | W/F | pass | `lyra down` took 0.8 s. The run was cancelled by `session_closed` and its cleanup succeeded. The containers are stopped, not removed, and the `app-db-data` volumes of both worktrees still exist. |
