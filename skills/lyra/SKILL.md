...
2. `lyra catalog --json` — the tools. Remember `catalog_revision`; later use `lyra catalog --if-revision N --json` and reuse your copy when `meta.not_modified` is true. The cache is only valid for the same workspace and query.
