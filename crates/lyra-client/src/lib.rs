//! Typed LIPC/1 client shared by the CLI and the TUI.
//!
...
use lyra_protocol::ids::{AbsolutePath, ClientId, HostEpoch};
...
use lyra_protocol::reply::{PublicReply, ReplyContext, WorkspaceRef};
use lyra_protocol::{ErrorCode, ErrorInfo, schemas};
use serde::Serialize;
...
pub async fn connect(paths: &WorkspacePaths, opts: &ConnectOptions) -> Result<Client, ClientError> {
    ...
    let mut client = Client {
        ...
        hello: HelloReply {
            ...
            catalog_revision: lyra_protocol::CatalogRevision::ZERO,
            state_revision: lyra_protocol::StateRevision::ZERO,
        },
    };
