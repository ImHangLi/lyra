...
/// Public schema names accepted by `lyra schema NAME`.
pub const NAMES: &[&str] = &[
    "workspace",
    "plugin",
    "local",
...
fn ipc_schema() -> Value {
    ...
    json!({
        ...
        "x-lyra-methods": methods,
        "x-lyra-public-reply": reply,
        "$defs": defs,
    })
}
...
pub fn schema(name: &str) -> Option<Value> {
    Some(match name {
        ...
        "invocation" => root::<crate::lpp::Invocation>(),
        "plugin-event" => root::<crate::lpp::PluginFrame>(),
