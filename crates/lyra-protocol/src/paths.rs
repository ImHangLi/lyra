//! Per-workspace file locations (§14.1). `lyra paths` is the public way to discover them.
//!
//! Development overrides: `LYRA_DATA_HOME` replaces `~/Library/{Application Support,Logs,Caches}`
//! with `<dir>/{state,logs,cache}`; `LYRA_RUNTIME_DIR` replaces `/tmp/lyra-<uid>`.
...
impl WorkspacePaths {
    pub fn new(root: AbsolutePath) -> Self {
        ...
        let wid = id.as_str();
        let (state_base, logs_base, cache_base) = match std::env::var_os("LYRA_DATA_HOME") {
            Some(d) => {
            ...
            None => {
                ...
                (
                    lib.join("Application Support/Lyra"),
                    lib.join("Logs/Lyra"),
                    lib.join("Caches/Lyra"),
                )
            }
        };
    ...
    pub fn to_data(&self) -> PathsData {
        let rows: [(PathClass, PathBuf, &str, bool, bool); 13] = [
            (
                ...
                self.lyra_dir.join("workspace.json"),
            ...
            (
                PathClass::Discovery,
                self.lyra_dir.join(".generated/discovery.json"),
            ...
            (
                ...
                "Segmented canonical run logs; read via `lyra logs`",
                true,
                false,
            ),
            (
