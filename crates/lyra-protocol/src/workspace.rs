...
impl Selected {
    pub fn lyra_dir(&self) -> PathBuf {
        self.root.as_path().join(".lyra")
    }
    pub fn is_setup(&self) -> bool {
        self.lyra_dir().join("workspace.json").is_file()
    }
}
...
pub fn select(project: Option<&Path>, cwd: &Path) -> Result<Selected, SelectError> {
    ...
    for dir in cwd.ancestors() {
        ...
        if dir.join(".lyra").join("workspace.json").is_file() {
            return Ok(selected(canonical(dir)?, SelectionReason::LyraConfig));
        }
    ...
    if let Ok(entries) = std::fs::read_dir(cwd) {
        for e in entries.flatten() {
            ...
            if (p.join(".git").exists()
                || p.join(".lyra").join("workspace.json").is_file()
                || has_marker(&p))
