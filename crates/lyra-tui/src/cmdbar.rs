//! The `:` command bar (§10.1, §12.3): runs one public `lyra` command for infrastructure work
...
pub fn split(line: &str) -> Result<Vec<String>, String> {
    ...
    while let Some(c) = chars.next() {
        match (quote, c) {
            ...
            (None, ';' | '&' | '|' | '<' | '>' | '`' | '$' | '(' | ')') => {
                return Err(format!(
                    "`{c}` is shell syntax; the command bar runs one lyra command, not a shell (quote it to pass it as text)"
                ));
            }
            (None, c) => {
                cur.push(c);
    ...
    if words.first().is_some_and(|w| w == "lyra") {
        words.remove(0);
    }
    match words.first().map(String::as_str) {
        None => Err("type a lyra command, for example `status` or `doctor`".into()),
...
pub fn hint(prefix: &str) -> String {
    ...
    if prefix.contains(' ') || matches.is_empty() {
        "Enter runs it (lyra … --json) · Esc cancels".into()
    } else {
        matches.join(" ")
    }
}

/// Runs `lyra --project ROOT --json WORDS...` and reports its reply.
pub fn run(root: String, words: Vec<String>, tx: Tx) {
    tokio::spawn(async move {
        let title = format!("lyra {}", words.join(" "));
