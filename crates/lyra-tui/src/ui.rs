...
use lyra_protocol::manifest::{ActionMode, ViewKind};
use lyra_protocol::run::{Lifecycle, LogStream, Outcome, RunRecord, RunResult};
    ...
    if app.items.is_empty() && app.views.is_empty() {
        match &app.catalog_error {
            ...
            None => empty_card(
                ...
                area,
                "No tools yet",
                "Run `lyra setup` and ask your agent to create them.",
            ),
        }
        return;
    }
...
fn extras_text(app: &App, a: &lyra_protocol::ids::ActionRef) -> String {
...
fn shown_result<'a>(app: &'a App, a: &lyra_protocol::ids::ActionRef) -> Option<&'a RunResult> {
