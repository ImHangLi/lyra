//! `lyra schedule ACTION on|off`: persist an interval schedule switch (§10.2).

use std::process::ExitCode;

use lyra_client::ConnectOptions;
use lyra_protocol::ipc::{ClientEnv, Method, ScheduleData, ScheduleSetParams};
...
pub fn set(ctx: &Ctx, action: &str, switch: Switch) -> ExitCode {
    block_on(async {
        let parsed = (|| {
            ...
            Ok::<_, lyra_protocol::ErrorInfo>(ScheduleSetParams {
                action_ref,
