...
/// The instruction a person gives their agent after `lyra setup`. No apostrophes, so it can
/// be quoted with single quotes in a shell.
const AGENT_PROMPT: &str = "Set up Lyra for this repository. Follow the lyra skill: run \
`lyra setup --json`, read the docs and scripts, create tools for the real dev commands, \
validate and apply them, run `lyra doctor`, verify one tool, and tell me what is ready and \
what is not.";
...
fn found_summary(facts: &[Fact]) -> String {
    use lyra_discovery::FactKind as K;
...
/// `lyra setup` for people: install the agent skills, then print the one command that lets
...
pub fn guided(project: Option<&Path>, agent: Option<AgentKind>) -> ExitCode {
    ...
    let cwd = match std::env::current_dir() {
        Ok(c) => c,
        Err(e) => {
            eprintln!("lyra setup: cannot read the current directory: {e}");
    ...
    let skills = match install_into(root, kind) {
        Ok(r) => r,
        Err(e) => {
            eprintln!("lyra setup: cannot install the agent skills: {e}");
    ...
    if selected.is_setup() {
        out.push_str(
            "\n  ✓ this project already has Lyra tools\n\nOpen the workbench with `lyra`. \
             To add a tool, ask your agent in plain words (the lyra-extend skill saves it).",
        );
        println!("{out}");
        return ExitCode::SUCCESS;
    }
    let facts = lyra_discovery::discover(root, false).discovery.facts;
    out.push_str(&format!("\n  ✓ {}", found_summary(&facts)));
    ...
    out.push_str(if copied {
        "\nThe prompt is on your clipboard. When the agent is done, run `lyra`."
    } else {
        "\nWhen the agent is done, run `lyra`."
    });
    println!("{out}");
    ExitCode::SUCCESS
}
