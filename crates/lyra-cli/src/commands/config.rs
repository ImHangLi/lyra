...
use lyra_protocol::ids::{AbsolutePath, CatalogRevision, RequestKey};
use lyra_protocol::ipc::{ConfigApplied, ConfigApplyParams, Empty, Method};
...
pub fn apply(ctx: &Ctx, draft: &Path, expected: u64, request_key: Option<String>) -> ExitCode {
    block_on(async {
        let parsed = (|| {
            ...
            Ok::<_, lyra_protocol::ErrorInfo>(ConfigApplyParams {
                draft_dir,
