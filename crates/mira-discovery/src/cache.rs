//! `<root>/.mira/.generated/discovery.json` (§14.1): the last result, the fingerprints of
...
use mira_protocol::{Digest, Timestamp};
...
pub fn cache_path(root: &Path) -> PathBuf {
    root.join(".mira").join(".generated").join("discovery.json")
}
...
pub enum CacheWrite {
    ...
    NotNeeded,
    Written,
    /// `.mira` or `.mira/.generated` exists but is not a plain directory; nothing is written.
    SkippedNotDirectory,
    Failed,
}
...
fn write_cache(root: &Path, file: &CacheFile) -> Result<(), (CacheWrite, String)> {
    ...
    let generated = mira.join(".generated");
    ensure_dir(&mira)?;
