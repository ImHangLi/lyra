//! LPP/1 stdout handling (§7.3, §7.5, §7.6): bounded LF framing, per-frame validation, and
...
use lyra_protocol::ids::ViewId;
use lyra_protocol::limits::MAX_LPP_FRAME_BYTES;
use lyra_protocol::lpp::{LineDecoder, PluginEvent, parse_frame};
...
/// What the runner needs to speak LPP/1 with one child.
pub struct Protocol {
    ...
    pub fn push(&mut self, bytes: &[u8]) -> Vec<FrameOut> {
        ...
        for line in self.decoder.push(bytes) {
            ...
            let line = match line {
                Ok(l) => l,
                Err(e) => {
                    out.push(self.error(e.code(), format!("invalid LPP/1 output: {e}"), None));
                    break;
                }
            };
            match parse_frame(&line) {
                ...
                Err(issues) => {
                    ...
                    out.push(FrameOut::Error {
                        error: ErrorInfo {
                            message: format!("invalid LPP/1 frame: {}", info.message),
                            ..info
                        },
                        view_hint: hint,
                    });
