...
use lyra_protocol::error::{ErrorCode, ErrorInfo, Issue, Issues};
use lyra_protocol::ids::{CatalogRevision, PluginId};
...
pub enum ConfigJob {
    ...
    /// Re-validates `.lyra` from disk (explicit `lyra reload` or a file notification).
    Reload,
}
...
impl ConfigCtl {
    pub fn new(paths: &WorkspacePaths) -> Self {
        ...
        Self {
            busy: false,
            lyra_watched: paths.lyra_dir.is_dir(),
            queue: VecDeque::new(),
...
fn start_watcher(
    ...
    let lyra = paths.lyra_dir.clone();
    let ignored = [lyra.join(".generated"), lyra.join(".drafts")];
    let mut watcher = notify::recommended_watcher(move |res: notify::Result<notify::Event>| {
        if let Ok(ev) = res {
            let relevant = ev
                .paths
                .iter()
                .any(|p| p.starts_with(&lyra) && !ignored.iter().any(|i| p.starts_with(i)));
            if relevant {
                let _ = tx.send(());
            }
        }
    ...
    .ok()?;
    // Watching the root non-recursively notices `.lyra` being created; `.lyra` itself recursively.
    let _ = watcher.watch(paths.root.as_path(), RecursiveMode::NonRecursive);
    if paths.lyra_dir.is_dir() {
        let _ = watcher.watch(&paths.lyra_dir, RecursiveMode::Recursive);
    }
    Some(watcher)
}
...
fn load_disk(lyra_dir: &Path) -> Result<ConfigSet, Issues> {
    let local = config::load_local(lyra_dir)?;
    config::load_config_dir(lyra_dir, local.as_ref())
}
...
fn write_atomic(target: &Path, bytes: &[u8]) -> std::io::Result<()> {
    ...
    let name = target
        ...
        .unwrap_or_default();
    let tmp = target.with_file_name(format!(".{name}.lyra-tmp-{}", std::process::id()));
...
/// Validates the draft, then copies its definitions into `.lyra`, then re-validates `.lyra`.
fn apply_draft(paths: &WorkspacePaths, draft: &Path) -> Result<ConfigSet, LoadFailure> {
    ...
    let lyra = std::fs::canonicalize(&paths.lyra_dir).unwrap_or(paths.lyra_dir.clone());
    if draft == lyra {
        return load_disk(&lyra).map_err(LoadFailure::Invalid);
    }
    let local = config::load_local(&paths.lyra_dir).map_err(LoadFailure::Invalid)?;
    ...
    let result = (|| {
        ...
        for rel in &set.workspace.plugins {
            ...
            copy_tree(&draft.join(rel), &paths.lyra_dir.join(rel), &mut written)?;
        }
        let target = paths.lyra_dir.join(config::WORKSPACE_FILE);
    ...
    load_disk(&paths.lyra_dir).map_err(|issues| LoadFailure::Incomplete {
        written,
        error: format!(
            "files were written but .lyra does not validate: {}",
...
async fn accept(
    ...
    set: ConfigSet,
    key: Option<(lyra_protocol::ids::RequestKey, CatalogRevision)>,
...
impl Actor {
    ...
    pub(super) fn fs_changed(&mut self) {
        if !self.cfg.lyra_watched && self.paths.lyra_dir.is_dir() {
            use notify::Watcher;
            if let Some(w) = self.cfg._watcher.as_mut() {
                self.cfg.lyra_watched = w
                    .watch(&self.paths.lyra_dir, notify::RecursiveMode::Recursive)
                    .is_ok();
            }
        }
    ...
    fn process_config_queue(&mut self) {
        ...
        match job {
            ...
            ConfigJob::Reload => {
                ...
                tokio::spawn(async move {
                    let dir = paths.lyra_dir.clone();
                    let loaded = tokio::task::spawn_blocking(move || {
                        if !dir.join(config::WORKSPACE_FILE).is_file() {
                            return Err(LoadFailure::Invalid(Issues(vec![Issue::new(
                                ErrorCode::NOT_SETUP,
                                "",
                                "no .lyra/workspace.json",
                            )])));
                        }
    ...
    pub(super) fn config_done(
        ...
        previous_hash: Option<lyra_protocol::ids::Digest>,
    ) {
        self.cfg.busy = false;
        match result {
            Ok(loaded) => {
                ...
                if is_apply_validation {
                    ...
                    if !not_setup {
                        ...
                        self.cfg.blocked = affected(
                            set.as_deref(),
                            &issues,
                            "configuration on disk is invalid; fix it and run `lyra reload`",
                        );
            ...
            Err(LoadFailure::Incomplete { written, error }) => {
                ...
                self.cfg.blocked = Blocked::All(
                    ErrorCode::CONFIG_APPLY_INCOMPLETE,
                    format!(
                        "the last configuration apply was incomplete ({error}); fix the files and run `lyra reload`"
                    ),
                );
    ...
    pub(crate) fn retiring_warnings(&self) -> Vec<Warning> {
        ...
        for run in self.runs.values() {
            ...
            out.push(Warning {
                code: lyra_protocol::ErrorCode::parse("DEFINITION_CHANGED".into())
