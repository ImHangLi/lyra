...
use lyra_protocol::config::ConfigSet;
use lyra_protocol::error::{ErrorCode, ErrorInfo};
use lyra_protocol::ids::RunId;
use lyra_protocol::ipc::LogPage;
use lyra_protocol::limits::KIB;
use lyra_protocol::reply::{ReplyContext, ReplyMeta};
use lyra_protocol::run::{Lifecycle, LogRecord, Outcome, RunProvenance, RunRecord};
use lyra_protocol::view::Freshness;
...
fn log_seq(v: Option<u64>) -> Option<lyra_protocol::ids::LogSeq> {
    v.and_then(|s| lyra_protocol::ids::LogSeq::new(s).ok())
}
