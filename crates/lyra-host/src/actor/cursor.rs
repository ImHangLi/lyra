...
use lyra_protocol::limits::MAX_CURSOR_BYTES;
...
pub(crate) fn decode(s: &str, kind: Kind, source: &str, filter: &str) -> Result<Cursor, ErrorInfo> {
    ...
    let bytes = base64::engine::general_purpose::URL_SAFE_NO_PAD
        .decode(s)
        .map_err(|_| invalid("invalid cursor: not a Lyra cursor"))?;
    let value: Value =
        serde_json::from_slice(&bytes).map_err(|_| invalid("invalid cursor: not a Lyra cursor"))?;
    ...
    if value.get("k").is_some() && value.get("v").and_then(Value::as_u64) != Some(VERSION) {
        return Err(ErrorInfo::new(
            ...
            "the cursor comes from an older Lyra format; restart from the first page",
        ));
    }
    let wire: Wire =
