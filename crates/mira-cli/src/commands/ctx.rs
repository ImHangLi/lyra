...
impl Ctx {
    pub fn select(&self) -> Result<Selected, ErrorInfo> {
        let cwd = std::env::current_dir()
            .map_err(|e| ErrorInfo::new(mira_protocol::ErrorCode::NOT_FOUND, e.to_string()))?;
